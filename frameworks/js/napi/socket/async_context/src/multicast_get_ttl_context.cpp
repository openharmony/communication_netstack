...
bool MulticastGetTTLContext::CheckParamsType(napi_value *params, size_t paramsCount)
{
    if (paramsCount == PARAM_NONE) {
        return true;
    }
...
void MulticastGetTTLContext::SetMulticastTTL(int ttl)
{
    ttl_ = ttl;
}

int MulticastGetTTLContext::GetMulticastTTL() const
{
    return ttl_;
}
...
std::string MulticastGetTTLContext::GetErrorMessage() const
{
    if (BaseContext::IsPermissionDenied()) {
