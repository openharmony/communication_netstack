...
bool GetSignatureAlgorithmsContext::CheckParamsType(napi_value *params, size_t paramsCount)
{
    ...
    if (paramsCount == PARAM_JUST_CALLBACK) {
        if (NapiUtils::GetValueType(GetEnv(), params[ARG_INDEX_0]) != napi_function) {
            NETSTACK_LOGE("GetSignatureAlgorithmsContext first param is not function");
            return false;
        }
        return true;
    }
