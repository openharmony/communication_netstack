...
bool HttpExec::SetInterface(CURL *curl, RequestContext *context)
{
    ...
    if (ret && !interfaceName.empty() && netId > 100) {
    ...
    std::list<sptr<NetManagerStandard::NetHandle>> netList, std::string &interfaceName, int32_t &netId)
{
    ...
    if (cellCount == 1 && netList.size() == 2) {
        NetManagerStandard::NetLinkInfo info;
        if (!cellularNetworks.empty() && cellularNetworks.size() == 1 &&
            NetManagerStandard::NetConnClient::GetInstance().GetConnectionProperties
            (*(cellularNetworks.front()), info) == NetManagerStandard::NETMANAGER_SUCCESS) {
