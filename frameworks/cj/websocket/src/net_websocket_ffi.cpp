...
EXTERN_C_START
    ...
    int64_t FfiOHOSWebSocketCreateWebSocket()
    {
        ...
        NETSTACK_LOGI("CJWebSocket Create CJWebsocketProxy success instance = %{public}lld.", instance->GetID());
    ...
    RetDataBool FfiOHOSWebSocketConnect(int64_t id, char* url, CWebSocketRequestOptions* opt)
    {
        ...
        NETSTACK_LOGI("CJWebSocket FfiOHOSWebSocketConnect instance = %{public}lld.", id);
        if (instance == nullptr) {
            // destroyed
    ...
    RetDataBool FfiOHOSWebSocketSend(int64_t id, CArrUI8 data, bool stringType)
    {
        RetDataBool ret = { .code = 0, .data = false };
        auto instance = FFIData::GetData<CJWebsocketProxy>(id);
        NETSTACK_LOGI("CJWebSocket FfiOHOSWebSocketSend instance = %{public}lld.", id);
        if (instance == nullptr) {
            // destroyed
    ...
    {
        RetDataBool ret = { .code = 0, .data = false };
        NETSTACK_LOGI("CJWebSocket FfiOHOSWebSocketClose instance = %{public}lld.", id);
        auto instance = FFIData::GetData<CJWebsocketProxy>(id);
        if (instance == nullptr) {
            // destroyed
            ret.code = WEBSOCKET_UNKNOWN_OTHER_ERROR;
    ...
    int32_t FfiOHOSWebSocketOnController(int64_t id, int32_t typeId, void (*callback)(CWebSocketCallbackData *data))
    {
        ...
        NETSTACK_LOGI("CJWebSocket FfiOHOSWebSocketOnController instance = %{public}lld.", id);
        auto instance = FFIData::GetData<CJWebsocketProxy>(id);
        if (instance == nullptr) {
            // destroyed
    ...
    {
        int32_t ret = WEBSOCKET_UNKNOWN_OTHER_ERROR;
        NETSTACK_LOGI("CJWebSocket FfiOHOSWebSocketOffController instance = %{public}lld.", id);
        auto instance = FFIData::GetData<CJWebsocketProxy>(id);
        if (instance == nullptr) {
            // destroyed
