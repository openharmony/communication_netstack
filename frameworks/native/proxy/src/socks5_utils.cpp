...
{
    if (Socket::NetAddress::Family::IPv4 == netAddress.GetFamily()) {
        return sizeof(sockaddr_in);
    }
    if (Socket::NetAddress::Family::IPv6 == netAddress.GetFamily()) {
...
{
    return Socket::ExecCommonUtils::PollSendData(socketId, data, size, addr, addrLen);
}

std::pair<bool, Socks5Buffer> Socks5Utils::Recv(int32_t socketId, sockaddr *addr, socklen_t addrLen)
{
    const int32_t bufferSize = Socket::ExecCommonUtils::ConfirmBufferSize(socketId);
    auto buf = std::make_unique<char[]>(bufferSize);
    constexpr int32_t pollTimeout{500};
    const int32_t timeoutMs = Socket::ExecCommonUtils::ConfirmSocketTimeoutMs(socketId, SO_RCVTIMEO, pollTimeout);
    while (true) {
