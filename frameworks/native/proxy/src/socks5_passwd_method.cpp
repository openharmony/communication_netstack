...
namespace NetStack {
namespace Socks5 {
bool Socks5PasswdMethod::RequestAuth(std::int32_t socketId, const TlsSocket::SecureData &username,
    const TlsSocket::SecureData &password, const Socks5ProxyAddress &proxy)
{
    if (username.Data() == nullptr || username.Length() == 0 ||
        password.Data() == nullptr || password.Length() == 0) {
