...
#define SOCKS5_MAX_USERNAME_LEN 255
#define SOCKS5_MAX_PASSWORD_LEN 255
...
bool Socks5MethodResponse::Deserialize(const void *data, size_t len)
{
    ...
    version_ = buffer[0];
    method_ = buffer[1];
    if (version_ != SOCKS5_VERSION) {
        NETSTACK_LOGE("version is not SOCKS5_VERSION!");
        return false;
    }
    return true;
}
...
{
    if (username_.Length() > SOCKS5_MAX_USERNAME_LEN || password_.Length() > SOCKS5_MAX_PASSWORD_LEN) {
        NETSTACK_LOGE("username or password length is too long!");
        return "";
    }
    std::string serialized;
    ...
    size_t pos = 2;
    if (memcpy_s(&serialized[pos], serialized.size() - pos, username_.Data(), username_.Length()) != EOK) {
    ...
    if (memcpy_s(&serialized[pos], serialized.size() - pos, password_.Data(), password_.Length()) != EOK) {
        NETSTACK_LOGE("memcpy_s failed!");
        return "";
    }
    return serialized;
...
bool Socks5AuthResponse::Deserialize(const void *data, size_t len)
{
    ...
    version_ = buffer[0];
    status_ = buffer[1];
    if (version_ != SOCKS5_SUBVERSION) {
        NETSTACK_LOGE("version is not SOCKS5_SUBVERSION!");
        return false;
    }
    return true;
}
...
static int CopyAddrToStr(Socks5AddrType addrType, std::string &destAddr, std::string &serialized, size_t pos)
{
    switch (addrType) {
        ...
        case Socks5AddrType::DOMAIN_NAME: {
            if (destAddr.length() > SOCKS5_MAX_DOMAIN_LEN) {
                NETSTACK_LOGE("domain name length is too long!");
                return -1;
            }
