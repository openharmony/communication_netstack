...
bool Socks5Instance::ConnectProxy()
{
    ...
    if (!Socket::ExecCommonUtils::NonBlockConnect(socketId_, options_->proxyAddress_.addr_, addrLen, timeoutMSec)) {
        if (errno == EISCONN) {
            return true;
        }
...
bool Socks5UdpInstance::RemoveHeader(void *data, size_t &len, int af)
{
    ...
    size_t headerLen;
    const size_t reservedFragAtypLen = 4; // 4 = RSV + FRAG + ATYP
    const size_t portLen = 2; // 2 = PORT
    switch (header.addrType_) {
        case Socks5::Socks5AddrType::IPV4:
            headerLen = reservedFragAtypLen + IPV4_LEN + portLen;
            break;
        case Socks5::Socks5AddrType::IPV6:
            headerLen = reservedFragAtypLen + IPV6_LEN + portLen;
            break;
        case Socks5::Socks5AddrType::DOMAIN_NAME:
            headerLen = reservedFragAtypLen + 1U + header.destAddr_.length() + portLen; // 1: DOMAIN_LEN
            break;
        default:
            NETSTACK_LOGE("unknown socks5 udp addr type");
            return false;
    }
