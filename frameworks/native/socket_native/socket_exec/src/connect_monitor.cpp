...
void ConnectMonitor::WakeUp()
{
    ...
    ssize_t ret;
    do {
        ret = write(eventFd_, &val, sizeof(val));
    } while (ret == -1 && errno == EINTR);
    if (ret != sizeof(val)) {
        NETSTACK_LOGE("WakeUp write eventfd failed, errno=%d", errno);
    }
}
...
void ConnectMonitor::MonitorLoop()
{
    auto handleEventFd = [this]() {
        ...
        if (ret != sizeof(val)) {
            NETSTACK_LOGE("connect monitor read eventfd failed, errno=%d", errno);
        }
    };
