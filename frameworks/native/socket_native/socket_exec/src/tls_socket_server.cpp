...
std::vector<std::string> SplitEscapedAltNames(std::string &altNames)
{
    ...
    while (offset != altNames.length()) {
        ...
        bool isJsonString = (nextQuote != std::string::npos) &&
                            (nextSep == std::string::npos || nextQuote < nextSep);
        if (isJsonString) {
...
#endif
    if (!BindWithRetry(addr, len, &addr4, &addr6, callback)) {
        return false;
    }
...
bool TLSSocketServer::BindWithRetry(sockaddr *addr, socklen_t len, sockaddr_in *addr4, sockaddr_in6 *addr6,
...
bool TLSSocketServer::Connection::Close()
{
    ...
    if (tlsContextServerPointer_ != nullptr) {
...
{
    bool valid = false;
    std::string reason = UNKNOWN_REASON;
    ...
    if (!dnsNames.empty() || index > 0) {
        ...
        if (!dnsNames.empty()) {
            ...
            if (pSubName != nullptr) {
                len = X509_NAME_get_text_by_NID(pSubName, NID_commonName, commonNameBuf, COMMON_NAME_BUF_SIZE);
            }
            if (len > 0) {
...
void TLSSocketServer::CallOnConnectCallback(const int32_t socketFd)
{
...
void TLSSocketServer::PollThread(const TlsSocket::TLSConnectOptions &tlsListenOptions)
{
    ...
    NETSTACK_LOGI("PollThread  start working %{public}d", isRunning_.load());
