...
--------------------------------------------------
/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/http_client/BUILD.gn
utils_include 21行
"//foundation/communication/netstack/frameworks/native/http/http_client/include",


/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/http_client/HttpClientTaskTest.cpp
...
/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/http_client/HttpClientTest.cpp
26行
...
/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/tlssocket/client/TlsCertificateTest.cpp
CaCertToStringTest后
...
/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/tlssocket/client/TlsKeyTest.cpp
230行
    TLSKey tlsKeyOpaque = TLSKey(structureData, OPAQUE, keyPass);
230    tlsKeyOpaque.opaque_ = EVP_PKEY_new();
...
{
    std::string rsa_pem = generateRSAKeyPEM();
    SecureData data(rsa_pem);
...
std::string generateECKeyPEM() {
    ...
    if (PEM_write_bio_ECPrivateKey(bio, ec_key, nullptr, nullptr, 0, nullptr, nullptr) != 1) {
        BIO_free(bio);
        EC_KEY_free(ec_key);
        EC_GROUP_free((EC_GROUP*)group);
        return "";
...
{
    std::string ec_pem = generateECKeyPEM();
    SecureData data(ec_pem);
    SecureData passPhrase;
    TLSKey tlsKey = TLSKey(data, passPhrase);
    tlsKey.DecodeData(data, passPhrase);
...
std::string generateUnsupportedKeyPEM() {
    ...
    if (EVP_PKEY_generate(ctx, &evp_pkey) != 1) {
    ...
    if (PEM_write_bio_PrivateKey(bio, evp_pkey, nullptr, nullptr, 0, nullptr, nullptr) != 1) {
        BIO_free(bio);
        EVP_PKEY_free(evp_pkey);
        return "";
    }
...
{
    std::string unsupported_pem = generateUnsupportedKeyPEM();
    SecureData data(unsupported_pem);
    SecureData passPhrase;
    TLSKey tlsKey = TLSKey(data, passPhrase);
    tlsKey.DecodeData(data, passPhrase);
...
/home/zhengsiwen/single_trunk_0226/foundation/communication/netstack/test/unittest/tlssocket/core/tls_context_server.test.cpp
最后加
