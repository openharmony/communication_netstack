...
HWTEST_F(NetStackCommonUtilsTest, Sha256sum01, TestSize.Level2)
{
    // sha256sum test - if you have a string $HEX, run
    // echo -n $HEX | sha256sum | cut -f1 -d' ' | xxd -r -p | base64
    // to get the expected result.
    std::string digest;
    bool ret = Sha256sum(nullptr, 0, digest);
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=");
}

HWTEST_F(NetStackCommonUtilsTest, Sha256sum02, TestSize.Level2)
{
    std::string digest;
    unsigned char input[] = "http://www.example.com";
    bool ret = Sha256sum(input, 22, digest);    // strlen(input) == 22
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "IQjbGhQclW+UWtg+yHzI2CmQokZbwAyQRTbEQesOuKs=");
}

HWTEST_F(NetStackCommonUtilsTest, Sha256sum03, TestSize.Level2)
{
    std::string digest;
    unsigned char input[] = "http://www.example.com";
    bool ret = Sha256sum(input, 22, digest);    // strlen(input) == 22
...
HWTEST_F(NetStackCommonUtilsTest, Sha256sum04, TestSize.Level2)
{
    std::string digest;
    unsigned char input[] = "http://www.example.com";
    bool ret = Sha256sum(input, 6, digest);    // "http:/"
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "38JXuftBqfwxj6EQQAxEoxboyvoA9bBeab20/djv6v0=");
}

HWTEST_F(NetStackCommonUtilsTest, Sha256sum05, TestSize.Level2)
{
    std::string digest;
    unsigned char input[] = "http://www.example.com";
    bool ret = Sha256sum(input, 23, digest);    // strlen(input) == 22
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "42xpgyicHEaVC9Z20Ii6gnRv5Lt4WGUPQ+lM/7j3lHA=");
}

HWTEST_F(NetStackCommonUtilsTest, Sha256sum06, TestSize.Level2)
{
    std::string digest;
    ...
    bool ret = Sha256sum(input, 32, digest);
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "8YciZWPLOnJyj11rJkl3zyRHPx54CusDJs4YSSGax3Y=");
}

HWTEST_F(NetStackCommonUtilsTest, Sha256sum07, TestSize.Level2)
{
    std::string digest;
    unsigned char input[1005];
    for (int i = 0; i < 1005; i++) {
        input[i] = 0x55;
    }
    bool ret = Sha256sum(input, sizeof(input), digest);
    EXPECT_TRUE(ret);
    EXPECT_STREQ(digest.c_str(), "9NYt3sDz3ZDqE4D6FqX/jcTFSyF0BlDySvxBIJA1UrA=");
}
