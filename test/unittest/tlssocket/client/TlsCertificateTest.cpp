...
{
    TLSCertificate tlsCertificate = TLSCertificate(g_clientFile, LOCAL_CERT);
    bool ret = tlsCertificate.CertificateFromPem(g_certificat, LOCAL_CERT);
    EXPECT_FALSE(ret);
...
{
    TLSCertificate tlsCertificate = TLSCertificate(g_clientFile, LOCAL_CERT);
    bool ret = tlsCertificate.CertificateFromDer(g_caCrtFile, LOCAL_CERT);
    EXPECT_FALSE(ret);
...
{
    TLSCertificate tlsCertificate = TLSCertificate(g_clientFile, LOCAL_CERT);
    tlsCertificate.CertificateFromDer(g_certificat, CA_CERT);
