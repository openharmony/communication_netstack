...
HWTEST_F(TlsSocketBranchTest, BranchTest5, TestSize.Level2)
{
    ...
    tlsSocket.OnError([](int32_t errorNumber, const std::string &errorString) {
        EXPECT_EQ(TLSSOCKET_ERROR_SSL_NULL, errorNumber);
    });
    tlsSocket.Connect(tlsConnectOptions, [](int32_t errCode) { EXPECT_EQ(TLSSOCKET_ERROR_SSL_NULL, errCode); });
    std::string getData;
    ...
    tlsSocket.Send(tcpSendOptions, [](int32_t errCode) { EXPECT_EQ(errCode, TLSSOCKET_ERROR_SSL_NULL); });
    tlsSocket.GetSignatureAlgorithms([](int32_t errCode, const std::vector<std::string> &algorithms) {
        EXPECT_EQ(errCode, TLSSOCKET_ERROR_SSL_NULL);
    });
