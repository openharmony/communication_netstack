...
HttpCacheStrategy::HttpCacheStrategy(HttpClientRequest &requestOptions) : requestOptions_(requestOptions)
{
    cacheRequest_.ParseRequestHeader(requestOptions_.GetHeaders());
...
bool HttpCacheStrategy::CouldUseCache()
{
    return requestOptions_.GetMethod() == HttpConstant::HTTP_METHOD_GET ||
           ...
           requestOptions_.GetHeaders().find(KEY_RANGE) != requestOptions_.GetHeaders().end();
}

CacheStatus HttpCacheStrategy::RunStrategy(HttpClientResponse &response)
{
    cacheResponse_.ParseCacheResponseHeader(response.GetHeaders());
...
bool HttpCacheStrategy::IsCacheable(const HttpClientResponse &response)
{
    if (!CouldUseCache()) {
        return false;
    }
    HttpCacheResponse tempCacheResponse;
    tempCacheResponse.ParseCacheResponseHeader(response.GetHeaders());
...
CacheStatus HttpCacheStrategy::RunStrategyInternal(HttpClientResponse &response)
{
